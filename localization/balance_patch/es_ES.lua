return {
    descriptions = {
        Joker = {
            j_mystic_summit = {
                text = {
                    "{C:chips}+#1#{} fichas cuando quedan",
                    "{C:attention}#2#{} descartes",
                },
            },
            j_8_ball = {
                text = {
                    "{C:green}#1# entre #2#{} probabilidades por cada",
                    "{C:attention}8{} jugado de generar {C:attention}#3#{} copia",
                    "de la carta {C:tarot}El loco{} cuando puntúa",
                    "{C:inactive}(Debe haber espacio)",
                },
            },
            j_seance = {
                text = {
                    "Si {C:attention}la mano de póker{} es una",
                    "{C:attention}#1#{}, genera una carta {C:spectral}espectral{}",
                    "{C:dark_edition}negativa{} al azar",
                },
            },
            j_hiker = {
                text = {
                    "Cada {C:attention}carta{} jugada",
                    "gana permanentemente",
                    "{C:mult}+#1#{} multi cuando puntúa",
                },
            },
            j_matador = {
                text = {
                    "Gana {C:money}$#1#{} cuando se active",
                    "una habilidad de la {C:attention}ciega jefe{}",
                },
            },
            j_superposition = {
                text = {
                    "Genera una copia {C:dark_edition}negativa{} de la",
                    "última carta de {C:tarot}tarot{} o {C:planet}planeta{} usada",
                    "si la mano contiene un {C:attention}as{}",
                    "y una {C:attention}escalera{}",
                },
            },
            j_flower_pot = {
                text = {
                    "Las cartas que puntúan con {C:attention}2{} palos dan {X:mult,C:white}X#1#{} multi",
                    "Las cartas que puntúan con {C:attention}3{} palos dan {X:mult,C:white}X#2#{} multi",
                    "Las cartas que puntúan con {C:attention}4{} palos dan {X:mult,C:white}X#3#{} multi",
                },
            },
            j_to_the_moon = {
                text = {
                    "Gana {C:money}$#1#{} extra de {C:attention}interés{}",
                    "por cada {C:money}$#2#{} que tengas",
                    "al final de la ronda",
                },
            },
        },
        Voucher = {
            v_magic_trick = {
                text = {
                    "Las {C:enhanced}cartas de juego mejoradas{}",
                    "se pueden comprar en la {C:attention}tienda{}",
                },
            },
            v_illusion = {
                text = {
                    "Las {C:enhanced}cartas mejoradas{} de la tienda",
                    "siempre tienen una {C:dark_edition}edición{}",
                    "y/o un {C:attention}sello{}",
                },
            },
            v_seed_money = {
                text = {
                    "Aumenta el límite de interés",
                    "ganado por ronda a {C:money}$#1#{}",
                },
            },
            v_money_tree = {
                text = {
                    "Aumenta el límite de interés",
                    "ganado por ronda a {C:money}$#1#{}",
                },
            },
        },
        Stake = {
            stake_blue = {
                text = {
                    "La tienda puede tener comodines {C:attention}perecederos{}",
                    "{s:0.8}Se aplica a todos los pozos anteriores",
                },
            },
            stake_orange = {
                text = {
                    "La tienda puede tener comodines {C:attention}de alquiler{}",
                    "{s:0.8}Se aplica a todos los pozos anteriores",
                },
            },
            stake_gold = {
                text = {
                    "Gana {C:money}$2{} de interés por cada {C:money}$10{}",
                    "El interés base tiene un límite de {C:money}$4{} por ronda",
                    "{s:0.8}Se aplica a todos los pozos anteriores",
                },
            },
        },
    },
    misc = {
        dictionary = {
            k_bbp_one_fool = "¡+1 El loco!",
            k_bbp_negative_only = "¡Copia negativa!",
        },
    },
}
